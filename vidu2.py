...
# ==========================================================
# B5+ (SUPERVISED) – LINEAR REGRESSION FORECAST (DỰ ĐOÁN)
# ==========================================================
...
# ===========================
st.markdown("---")
st.subheader("📈 Dự đoán giá vàng (Linear Regression)")

# 1) Chọn horizon + tỷ lệ test
horizon = st.selectbox("Chọn bước dự đoán (t+h, theo NGÀY)", [1, 7, 14, 30, 60, 90, 180, 365], index=3)
test_ratio = st.slider("Tỷ lệ test (time split)", 0.1, 0.4, 0.2, 0.05)

# 2) Tạo dataset supervised từ chuỗi thời gian (dựa trên feature lag/rolling)
...
c3.metric("Test size", f"{len(test_df)}")
c4.metric("Horizon", f"t+{horizon} ngày")

# ===========================
# GIẢI THÍCH BẢN CHẤT MÔ HÌNH (RÕ RÀNG CHO BÁO CÁO)
# ===========================
with st.expander("📌 Bản chất mô hình đang dự đoán như thế nào? (đọc để hiểu đúng)"):
    st.write(f"""
- **Dữ liệu là theo ngày (daily)**: mỗi dòng là 1 ngày.
- Bạn đang làm bài toán **Supervised Learning** bằng cách tạo nhãn:
  - `y = Close(t + {horizon})`
- Mô hình **không nhìn tương lai**. Nó chỉ dùng thông tin quá khứ thông qua các feature:
  - `Close(t-1), Close(t-7), ...` (lag features)
  - `MA_7, MA_14, MA_30, STD_7` (rolling features)
  - `Volume` (nếu có)
- Dự đoán trong biểu đồ "Actual vs Predicted" là **dự đoán trên tập test (đoạn cuối lịch sử)**.
- Nếu bạn bật Forecast tương lai, đó là **dự báo sau ngày cuối dataset** (future forecasting).
""")

# ===========================
# VISUAL 1: ACTUAL vs PREDICTED (TEST)
# ===========================
st.write("### 📊 Actual vs Predicted (trên tập test – kiểm tra độ khớp)")

fig, ax = plt.subplots(figsize=(12, 4))
ax.plot(test_df["Date"], y_test.values, label="Actual", linewidth=2)
ax.plot(test_df["Date"], y_pred, label="Predicted", linewidth=2)
ax.set_title(f"Actual vs Predicted (Forecast t+{horizon} ngày)")
ax.set_xlabel("Date")
...
# ===========================
# HỆ SỐ (COEFFICIENTS) - HIỂN THỊ DỄ HIỂU HƠN
# ===========================
st.write("### 🧠 Đóng góp của feature (Linear Regression Coefficients)")
...
st.caption("Gợi ý đọc: |Coef| càng lớn → feature ảnh hưởng mạnh hơn (theo hướng + hoặc -).")
st.dataframe(coef_df.drop(columns=["AbsCoef"]).head(20), use_container_width=True)

# Vẽ top 15 hệ số theo độ lớn tuyệt đối
topk = min(15, len(coef_df))
fig2, ax2 = plt.subplots(figsize=(10, 5))
ax2.barh(coef_df["Feature"].head(topk)[::-1], coef_df["Coef"].head(topk)[::-1])
ax2.set_title("Top feature coefficients (Linear Regression)")
ax2.set_xlabel("Coefficient")
st.pyplot(fig2)

# ===========================
# DỰ BÁO TƯƠNG LAI (FUTURE FORECAST) - DỰ BÁO XA HƠN & TRỰC QUAN HƠN
# ===========================
st.markdown("---")
st.subheader("🔮 Dự báo tương lai (sau ngày cuối dataset)")

do_forecast = st.checkbox("Bật dự báo tương lai (multi-step forecast)", value=True)

if do_forecast:
    # Số ngày muốn dự báo
    n_steps = st.slider("Số ngày muốn dự báo tới tương lai (theo NGÀY giao dịch)", 7, 365, 90, 7)

    # Chọn dùng ngày lịch hay ngày giao dịch (business day)
    use_business_days = st.checkbox("Dùng ngày giao dịch (Business days, bỏ T7/CN)", value=True)

    # --- Hàm tạo 1 dòng feature từ lịch sử close/volume gần nhất ---
    def build_features_from_history(hist_df: pd.DataFrame) -> pd.DataFrame:
        """
        hist_df: DataFrame đã có cột Close/Last và Volume (nếu có), sắp theo Date tăng dần.
        Trả về 1 row DataFrame chứa các feature giống make_lr_dataset().
        """
        row = {}
    ...
    # --- Recursive forecast: dự đoán từng ngày kế tiếp ---
    ...
    # NOTE: Khi forecast nhiều bước, Volume tương lai không có.
    # Ở đây ta giữ volume features bằng cách: dùng vol_lag_1 và vol_ma_7 theo lịch sử (không cập nhật volume mới).
    # Điều này đúng “bản chất”: bạn không có Volume tương lai => mô hình dựa chủ yếu vào cấu trúc Close.
    for dt in future_dates:
        x_next = build_features_from_history(temp_hist)

        # nếu có NaN do thiếu lịch sử (đầu chuỗi), bỏ dự báo
        if x_next[feat_cols].isnull().any(axis=1).iloc[0]:
            preds.append(np.nan)
        else:
            y_next = lr.predict(x_next[feat_cols])[0]
            preds.append(y_next)

        # cập nhật lịch sử bằng Close dự đoán (recursive)
        new_row = {"Date": pd.Timestamp(dt), "Close/Last": preds[-1]}
        if "Volume" in temp_hist.columns:
            new_row["Volume"] = temp_hist["Volume"].iloc[-1]  # giữ volume cuối cùng (placeholder)
    ...
    st.write("### 📋 Bảng dự báo (Forecast table)")
    st.dataframe(forecast_df.head(30), use_container_width=True)

    # --- Visual 2: Lịch sử + dự báo (rất trực quan) ---
    st.write("### 📈 Lịch sử + dự báo tương lai (History + Forecast)")

    # lấy 1 đoạn lịch sử gần để vẽ cho đẹp (vd 240 ngày)
    lookback = st.slider("Số ngày lịch sử hiển thị trên biểu đồ", 60, 600, 240, 30)
    hist_plot = df[["Date", "Close/Last"]].copy().sort_values("Date").tail(lookback)

    fig3, ax3 = plt.subplots(figsize=(13, 5))
    ax3.plot(hist_plot["Date"], hist_plot["Close/Last"], label="Historical Close", linewidth=2)
    ax3.plot(forecast_df["Date"], forecast_df["Forecast_Close"], label="Forecast (LR)", linewidth=2)
    ax3.axvline(hist_plot["Date"].iloc[-1], linestyle="--", linewidth=1, label="Forecast start")
    ax3.set_title(f"Forecast {len(forecast_df)} ngày tới (Linear Regression, recursive)")
    ax3.set_xlabel("Date")
    ax3.set_ylabel("Close/Last")
    ax3.legend()
    plt.xticks(rotation=45)
    st.pyplot(fig3)

    # --- Gợi ý diễn giải đúng bản chất ---
    with st.expander("⚠️ Lưu ý quan trọng để diễn giải đúng (đúng bản chất)"):
        st.write("""
- Dự báo tương lai ở đây là **multi-step recursive**: dự đoán ngày kế tiếp rồi dùng chính kết quả đó để dự đoán ngày tiếp theo.
- Vì vậy **sai số có xu hướng tích lũy** khi dự báo xa (3–12 tháng).
- Dataset theo **ngày giao dịch**, nên bật “Business days” sẽ hợp lý hơn (bỏ T7/CN).
- Volume tương lai không có → đang dùng volume cuối cùng làm placeholder; muốn chuẩn hơn thì:
  - bỏ Volume khỏi feature khi forecast xa, hoặc
  - xây thêm model dự báo Volume trước rồi đưa vào (phức tạp hơn).
""")
